...
if [ -n $(manila type-list | grep -q 'default_share_type') ]; then
    echo 'add default_share_type'
    manila type-create default_share_type True
fi

if [ -n $(manila share-network-list | grep -q 'test_share_network') ]; then
...
if [ -n $(openstack flavor list | grep -q 'manila-service-flavor') ]; then
