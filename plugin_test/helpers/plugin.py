...
def enable_plugin_manila(cluster_id, fuel_web_client, driver='generic',
    ...
    assert_true(
        fuel_web_client.check_plugin_exists(cluster_id),
        "Plugin couldn't be enabled. Check plugin version.")
    if driver == 'generic':
        assert_true(fuel_web_client.check_plugin_exists(cluster_id),
                    "Plugin settings couldn't be enabled.Check plugin.")
        options = {'metadata/enabled': True, 'use-netapp-drive/value': True}
    ...
    if driver == 'nettapp':
        assert_true(fuel_web_client.check_plugin_exists(cluster_id),
                    "Plugin settings couldn't be enabled.Check plugin.")
        options = {'metadata/enabled': True, 'use-generic-drive/value': True}
        for name, value in kwargs.items():
            options[name] = value
        fuel_web_client.update_plugin_data(cluster_id, plugin_name, options)
