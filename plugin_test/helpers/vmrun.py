...
class Vmrun(object):
    ...
    def register(self):
        ...
        cmd.extend(['listRegisteredVM'])
        command_to_run = ' '.join(cmd)
        logger.info(command_to_run)
        # check registered vm's
        if self.path.split(" ")[1] not in \
                subprocess.check_output([command_to_run,grep], shell=True):
            cmd = self.__create_vrun_command()
    ...
    def register_revert_and_setup(self, snapshot_name):
        ...
        command_to_run = ' '.join(cmd)
        logger.info('{}'.format(command_to_run))
        assert_true(subprocess.check_call(command_to_run, shell=True) == 0,
    ...
    def set_config(self):
        ...
        assert_true(subprocess.check_call(
            ['ls -ld', '/etc/vmware/hostd/vmInventory.xml'],
            shell=True) == 0,
            "couldn't find vmInventory config file")

        assert_true(subprocess.check_call(
            ['sudo chmod 777 /etc/vmware/hostd/vmInventory.xml'],
            shell=True) == 0,
            "couldn't set permisson for vmInventory config file")
    ...
    def __set_bridges(self):
        ...
        cmd = 'brctl show {0}'.format(private_bridge_name.bridgeName())
        if 'vmnet3' not in subprocess.check_output(cmd, shell=True):
        ...
        cmd = 'brctl show {0}'.format(private_bridge_name.bridgeName())
        if 'vmnet4' not in subprocess.check_output(cmd, shell=True):
            cmd = 'sudo brctl  addif {1} {0}'.format(
                'vmnet4',
        ...
        cmd = 'brctl show {0}'.format(private_bridge_name.bridgeName())
        if 'vmnet2' not in subprocess.check_output(cmd, shell=True):
            cmd = 'sudo brctl  addif {1} {0}'.format(
                'vmnet2',
