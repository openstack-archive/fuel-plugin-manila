...
from helpers.settings import MANILA_PLUGIN_VERSION
from helpers.settings import plugin_name
    ...
    def manila_install(self):
        ...
        Scenario:
            ...
            6. Attempt to remove enabled plugin.
                Verify that plugin cannot be removed when it already enabled, run command 'fuel plugins'.
            7.Disable  plugin
        ...
        self.show_step(1)
        plugin.prepare_manila_plugin(self.ssh_manager.admin_ip)
        ...
        # check name
        assert_true(
            MANILA_PLUGIN_VERSION in output,
            "Plugin version '{0}' not found.".format(MANILA_PLUGIN_VERSION)
        )
        # check version
        assert_true(
            plugin_name in output,
            "Plugin '{0}' is not installed.".format(plugin_name)
        )
        ...
        self.show_step(6)
        cmd = 'fuel plugins --remove ' + plugin_name + '==' + MANILA_PLUGIN_VERSION
        output = self.ssh_manager.execute_on_remote(
            ip=self.ssh_manager.admin_ip,
            cmd=cmd,
